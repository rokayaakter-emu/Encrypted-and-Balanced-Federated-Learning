...
def init_server(model_type, dataset_name, input_shape, max_flow_len):
    ...
    if model_type == 'cnn':
        server['model'] = CNNModel('cnn', input_shape, kernels=KERNELS, kernel_rows=min(3, max_flow_len),
                                   kernel_col=features)
    elif model_type == 'mlp':
        ...
        print(
            "Error: Please use option \"model\" to indicate a model type (mlp or cnn), or to provide a pretrained model in h5 format")
        return None
